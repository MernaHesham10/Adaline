...
def ProcessingUserDataInput(firstFeature, secondFeature, firstClass, secondClass, addBias, numberOfEpochs, learningRate, error):
    ...
    def ChooseAndShuffle(firstClass, secondClass):
        ...
        for readingEachLine in readingLines:
            ...
            if i == 0:
                ...
                recordsDict = dict(bill_length_mm=bill_length_mm, bill_depth_mm=bill_depth_mm,
                                   flipper_length_mm=flipper_length_mm, gender=gender, body_mass_g=body_mass_g,
                                   species=speciesClass, Index=i)

                readingLinesRecords.append(recordsDict)
                readingLinesMod = i % 50
                i = i + 1
                recordsDictLine = recordsDict["bill_length_mm"] + ',' + recordsDict["bill_depth_mm"] + ',' + \
                                  recordsDict["flipper_length_mm"] + ',' + recordsDict["gender"] + ',' + recordsDict[
                                      "body_mass_g"] + ',' + recordsDict[
                                      "species"] + '\n'  # to change the format of the dict
                if ((readingLinesMod < 31 and readingLinesMod != 0) and (
                        speciesClass == firstClassTemp or speciesClass == secondClassTemp)):
                    with open("train.txt", "a") as trainfile:
                        trainfile.writelines(recordsDictLine)

                elif ((readingLinesMod >= 31 or readingLinesMod == 0) and (
                        speciesClass == firstClassTemp or speciesClass == secondClassTemp)):
    ...
    def AdalineCalculation(inputMatrix, weightMatrix, speciesClass, numberOfEpochs, learningRate, error):
        secondLoss = 0
        maxCheckError = 1000000000000

        while (maxCheckError > error):
            for i in range(0, 60):
                input_Matrix = np.empty([1, 3])
                input_Matrix[0][0] = inputMatrix[i][0]
                input_Matrix[0][1] = inputMatrix[i][1]
                input_Matrix[0][2] = inputMatrix[i][2]

                netMatrix = np.dot(input_Matrix, weightMatrix)

                loss = speciesClass[i] - netMatrix
                term_Value = np.dot(learningRate, loss)
                weightMatrix[0][0] = weightMatrix[0][0] + (np.dot(term_Value, input_Matrix[0][0]))
                weightMatrix[1][0] = weightMatrix[1][0] + (np.dot(term_Value, input_Matrix[0][1]))
                weightMatrix[2][0] = weightMatrix[2][0] + (np.dot(term_Value, input_Matrix[0][2]))
            ...
            for i in range(0, 60):
                ...
                secondLoss += np.nan_to_num(((speciesClass[i] - netMatrix) * (speciesClass[i] - netMatrix) / 2))

            maxCheckError = (secondLoss / 60)
            numberOfEpochs += 1
            secondLoss = 0

        print("max Check Error Value = ", maxCheckError, "\t Epochs is: ", numberOfEpochs)
    ...
    def AnotherInputMatrixConstructor(firstFeature, secondFeature, bias):
        ...
        if bias == True:
            enteredBias_CheckInputValue = 1

        elif bias == False:
            enteredBias_CheckInputValue = 0

        for i in range(0, 40):
            AnotherInputMatrix[i][0] = enteredBias_CheckInputValue
    ...
    def test(anotherInputMatrix, weightMatrix, target_List):
        accuracy = 0
        firstClass_True = 0
        secondClass_True = 0
        firstClass_False = 0
        ...
        for i in range(0, 40):
            ...
            if (target_List[i] == normaizedValue2 and target_List[i] == 1):
                firstClass_True += 1
                accuracy += 1

            elif (target_List[i] == normaizedValue2 and target_List[i] == -1):
                secondClass_True += 1
                accuracy += 1
    ...
    def main(firstFeature, secondFeature, firstClass, secondClass, addBias, numberOfEpochs, learningRate, error):
        ...
        updatedWeightMatrix = AdalineCalculation(inputMatrix, weightMatrix, CalSpeciesClass, numberOfEpochs, learningRate, error)

        # for test
        firstFeature_Test, secondFeature_Test, speciesClass_Test = ChooseFeatures(firstFeature, secondFeature, firstClass, secondClass, "test.txt")
        anotherInputMatrix = AnotherInputMatrixConstructor(firstFeature_Test, secondFeature_Test, addBias)
        acuuracy, confusionMatrix = test(anotherInputMatrix, updatedWeightMatrix, speciesClass_Test)

        print('Accuracy = ', int(acuuracy), '%', '\n', 'Confusion Matrix = \n', confusionMatrix, '\n')
        DisplayingProcessDataInput(int(acuuracy), confusionMatrix)

        # for drawing line
        draw_line(firstFeature_Test, secondFeature_Test, speciesClass_Test, updatedWeightMatrix)

    main(firstFeature, secondFeature, firstClass, secondClass, addBias, numberOfEpochs, learningRate, error)
...
def GettingAllUserDataInput():
    ...
    savedSecondFeature = secondFeature_Tuple.index(str(secondFeature.get()))
    ...
    if (firstFeature_Tuple.index(str(firstFeature.get())) == secondFeature_Tuple.index(str(secondFeature.get()))):
    ...
    print (firstFeature_Tuple[savedFirstFeature], secondFeature_Tuple[savedSecondFeature], firstClass_Tuple[savedFirstClass], secondClass_Tuple[savedSecondClass], savedAddBiasEntered_Check, savedNumberOfEpochs, savedLearningRate)

    ProcessingUserDataInput(firstFeature_Tuple[savedFirstFeature], secondFeature_Tuple[savedSecondFeature], firstClass_Tuple[savedFirstClass], secondClass_Tuple[savedSecondClass], savedAddBiasEntered_Check, savedNumberOfEpochs, savedLearningRate)
...
secondFeature_Tuple = ('bill_length_mm', 'bill_depth_mm', 'flipper_length_mm', 'gender', 'body_mass_g')

secondFeature = StringVar()
secondFeature_Combox = Combobox(tkinterWindow, textvariable=secondFeature, font=('Lucida Calligraphy', 10))
secondFeature_Combox['values'] = secondFeature_Tuple
...
####################### Enter Error Value ############################
error = tk.StringVar()
error_Label = tk.Label(tkinterWindow, text="Enter Error Value", font=('Lucida Calligraphy', 10), pady=10)
error_Entry = Entry(tkinterWindow, textvariable=error, font=('Lucida Calligraphy', 10))
error_Entry.insert(END, 0)

error_Label.grid(column=1, row=66)
error_Entry.grid(column=2, row=66)

################# ADD Bias ########################
addBias = IntVar()
addBias_Checkbox = Checkbutton(tkinterWindow, text="Add Bias", variable=addBias, font=('Lucida Calligraphy', 10), pady=10, padx=50)
addBias_Checkbox.grid(column=2, row=68)
...
###################### Buttons ###############################
makingPlots_Btn = Button(tkinterWindow, text='Show Data Plots', command=MakingWholeDataSetPlots, font=('Lucida Calligraphy', 11), pady=10, padx=50, bg='white', fg='blue')
